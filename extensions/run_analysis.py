    ...
    with open(queries_path, 'w', encoding='utf-8') as f:
        ...
        for i, chain in enumerate(most_complete_chains, 1):
            ...
            if len(path_nodes) >= 2:
                ...
                f.write(f"RETURN path;\n\n")
                
                f.write(f"-- 2. 查询链路上的所有节点（图形显示）\n")
                f.write(f"MATCH path = shortestPath((source)-[*1..{max_path_length}]->(target))\n")
                f.write(f"WHERE elementId(source) = '{source_node_id}' AND elementId(target) = '{target_node_id}'\n")
                f.write(f"RETURN path;\n\n")
                
                f.write(f"-- 3. 查询链路上的脚本组件（完全展开图形显示）\n")
                f.write(f"MATCH path = shortestPath((source)-[*1..{max_path_length}]->(target))\n")
                f.write(f"WHERE elementId(source) = '{source_node_id}' AND elementId(target) = '{target_node_id}'\n")
                f.write(f"WITH nodes(path) as path_nodes\n")
                f.write(f"UNWIND path_nodes as node\n")
                f.write(f"WHERE 'Script' IN labels(node)\n")
                f.write(f"MATCH (n)-[r]-(m)\n")
                f.write(f"WHERE n = node\n")
                f.write(f"RETURN n, r, m;\n\n")
                
                f.write(f"-- 4. 查询链路上的函数组件（完全展开图形显示）\n")
                f.write(f"MATCH path = shortestPath((source)-[*1..{max_path_length}]->(target))\n")
                f.write(f"WHERE elementId(source) = '{source_node_id}' AND elementId(target) = '{target_node_id}'\n")
                f.write(f"WITH nodes(path) as path_nodes\n")
                f.write(f"UNWIND path_nodes as node\n")
                ...
                f.write(f"-- 5. 查询链路上的变量组件（完全展开图形显示）\n")
                f.write(f"MATCH path = shortestPath((source)-[*1..{max_path_length}]->(target))\n")
                f.write(f"WHERE elementId(source) = '{source_node_id}' AND elementId(target) = '{target_node_id}'\n")
                f.write(f"WITH nodes(path) as path_nodes\n")
                f.write(f"UNWIND path_nodes as node\n")
                ...
                f.write(f"-- 6. 查询链路上的所有关系（完全展开图形显示）\n")
                f.write(f"MATCH path = shortestPath((source)-[*1..{max_path_length}]->(target))\n")
                f.write(f"WHERE elementId(source) = '{source_node_id}' AND elementId(target) = '{target_node_id}'\n")
                f.write(f"WITH nodes(path) as path_nodes\n")
                f.write(f"UNWIND path_nodes as node\n")
                ...
                f.write(f"-- 8. 查询链路完整图形（包含所有节点和关系）\n")
                f.write(f"MATCH path = shortestPath((source)-[*1..{max_path_length}]->(target))\n")
                f.write(f"WHERE elementId(source) = '{source_node_id}' AND elementId(target) = '{target_node_id}'\n")
                f.write(f"WITH nodes(path) as path_nodes, relationships(path) as path_rels\n")
                f.write(f"UNWIND path_nodes as node\n")
                f.write(f"UNWIND path_rels as rel\n")
                f.write(f"RETURN startNode(rel) as source, rel as relationship, endNode(rel) as target;\n\n")
                
                f.write(f"-- 9. 查询链路节点的完整关系网络（推荐使用）\n")
                f.write(f"MATCH path = shortestPath((source)-[*1..{max_path_length}]->(target))\n")
                f.write(f"WHERE elementId(source) = '{source_node_id}' AND elementId(target) = '{target_node_id}'\n")
                f.write(f"WITH nodes(path) as path_nodes\n")
                f.write(f"UNWIND path_nodes as node\n")
                ...
                f.write(f"-- 10. 查询链路节点的2跳关系网络（最完整显示）\n")
                f.write(f"MATCH path = shortestPath((source)-[*1..{max_path_length}]->(target))\n")
                f.write(f"WHERE elementId(source) = '{source_node_id}' AND elementId(target) = '{target_node_id}'\n")
                f.write(f"WITH nodes(path) as path_nodes\n")
                f.write(f"UNWIND path_nodes as node\n")
                f.write(f"MATCH (n)-[r1]-(m1)-[r2]-(m2)\n")
                f.write(f"WHERE n = node\n")
                f.write(f"RETURN n, r1, m1, r2, m2;\n\n")
        ...
        # 添加通用的影响链查询语句
        f.write("-- ========================================\n")
        f.write("-- 通用影响链查询语句（图形显示）\n")
        f.write("-- ========================================\n\n")
        
        f.write("-- 查询所有影响链（图形显示）\n")
        f.write("MATCH path = (source)-[r*1..5]->(target)\n")
        f.write("WHERE source <> target\n")
        f.write("RETURN path\n")
        f.write("ORDER BY length(path) DESC\n")
        f.write("LIMIT 10;\n\n")
        
        f.write("-- 查询包含脚本的影响链（图形显示）\n")
        f.write("MATCH path = (source)-[r*1..5]->(target)\n")
        ...
        f.write("-- 查询函数调用链（图形显示）\n")
        f.write("MATCH path = (source:Function)-[r:CALLS*1..3]->(target)\n")
        f.write("WHERE source <> target\n")
        f.write("RETURN path\n")
        f.write("ORDER BY length(path) DESC\n")
        f.write("LIMIT 5;\n\n")
        
        f.write("-- 查询变量依赖链（图形显示）\n")
        f.write("MATCH path = (source)-[r:DEFINES|USES*1..3]->(target)\n")
        f.write("WHERE source <> target\n")
        f.write("RETURN path\n")
        f.write("ORDER BY length(path) DESC\n")
        f.write("LIMIT 5;\n\n")
        
        f.write("-- 查询特定文件的影响（图形显示）\n")
        f.write("MATCH (n)-[r]-(m)\n")
        f.write("WHERE n.file_path CONTAINS 'getSupportFilePath.m' OR m.file_path CONTAINS 'getSupportFilePath.m'\n")
        f.write("RETURN n, r, m;\n\n")
        
        f.write("-- 查询特定文件节点的完整关系网络（推荐使用）\n")
        f.write("MATCH (n)\n")
        f.write("WHERE n.file_path CONTAINS 'getSupportFilePath.m'\n")
        f.write("MATCH (n)-[r]-(m)\n")
        f.write("RETURN n, r, m;\n\n")
        
        f.write("-- 查询所有Script节点的完整关系网络\n")
        f.write("MATCH (n:Script)-[r]-(m)\n")
        f.write("RETURN n, r, m\n")
        f.write("LIMIT 50;\n\n")
        
        f.write("-- 查询所有Function节点的完整关系网络\n")
        f.write("MATCH (n:Function)-[r]-(m)\n")
        f.write("RETURN n, r, m\n")
        f.write("LIMIT 50;\n\n")
        
        f.write("-- 查询所有Variable节点的完整关系网络（限制数量）\n")
        f.write("MATCH (n:Variable)-[r]-(m)\n")
        f.write("RETURN n, r, m\n")
        f.write("LIMIT 30;\n\n")
        
        f.write("-- 查询CALLS关系的完整网络\n")
        f.write("MATCH (n)-[r:CALLS]->(m)\n")
        f.write("RETURN n, r, m\n")
        f.write("LIMIT 30;\n\n")
        
        f.write("-- 查询DEFINES关系的完整网络\n")
        f.write("MATCH (n)-[r:DEFINES]->(m)\n")
        f.write("RETURN n, r, m\n")
        f.write("LIMIT 30;\n\n")
        
        f.write("-- 查询USES关系的完整网络\n")
        f.write("MATCH (n)-[r:USES]->(m)\n")
        f.write("RETURN n, r, m\n")
        f.write("LIMIT 30;\n\n")
